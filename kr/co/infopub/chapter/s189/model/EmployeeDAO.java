...
public class EmployeeDAO extends DataBase{
	public int getEmployeesTotal() throws SQLException{
		...
		try {
			...
			log("4/6 getEmployesTotal Success!!!");
	...
	public List<DepartmentDto> findAllDepartments2() throws SQLException{
		...
		PreparedStatement psmt = null;
		ResultSet rs = null;
		//ObservableList<DepartmentDto> empList = FXCollections.observableArrayList();
		List<DepartmentDto> empList = new ArrayList<>();
		try {
			conn = getConnection();
			psmt = conn.prepareStatement(SQL);
			...
			log("5/6 findAllDepartments Error!!!",e);
			throw e;
		}finally {
			close(conn, psmt, rs);
		}
	...
	public List<DepCountDto> findAllDepCounts() throws SQLException{
		...
		+ " NVL(D.DEPARTMNET_NAME, 'NOTYET') DEPARTMENT_NAME             "
		+ " FROM EMPLOYEES E, DEPARTMENTS D                  "
		+ " WHERE E.DEPARTMENT_ID = D.DEPARTMENT_ID(+)               "
		+ " GROUP BY E.DEPARTMENT_ID, D.DEPARTMENT_NAME                "
		...
		try {
			...
			log("5/6 findAllDepCounts Error!!!",e);
			throw e;
		}finally {
			close(conn, psmt, rs);
		}
	...
	public List<EmployeeDto> findTreeManagerInEmployee() throws SQLException{
		...
		try {
			...
			log(SQL, "findTreeManagerInEmployee Success!!!");
			rs = psmt.executeQuery();
			...
			log("5/6 findTreeManagerInEmployee Error!!!",e);
			throw e;
		} finally {
			close(conn, psmt, rs);
		}
	...
	public int getTreeMaxLevel() throws SQLException{
		String SQL = ""
			...
			+ " FROM EMPLOYEES                             "
			+ " START WITH MANAGER_ID IS NULL              "
			+ " CONNECT BY PRIOR EMPLOYEE_ID IS MANAGER_ID              ";
		Connection conn = null;
		PreparedStatement psmt = null;
		ResultSet rs = null;
		int count = 0;
