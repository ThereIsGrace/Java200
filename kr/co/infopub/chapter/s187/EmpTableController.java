...
public class EmpTableController {
	...
	@FXML
	void initialize() {
		//칼러뫄 매핑, Callback -> call을 구현해야 함
		empIdColumn.setCellValueFactory(
				new Callback<TableColumn.CellDataFeatures<Employee,Integer>,
				ObservableValue<Integer>>(){

					@Override
					public ObservableValue<Integer> call(CellDataFeatures<Employee, Integer> cellData) {
	...
	private void searchEmployees(ActionEvent actionEvent) throws SQLException{
		try {
			...
			System.out.println("employeeDAO.findAllDEmployees()에서 예외 발생\n"+ e);
		}
	}
}
