	...
	void initialize() {
		...
		employeeTable.setOnMouseClicked(e -> {
			if(employeeTable.getSelectionModel().getSelectedItem()!=null) {
				...
				//lbhello.setText(user.getEmployeeId()+" "+ 
				//user.getFirstName()+" "+user.getLastName());
			}
		});
		...
		if(cbdepartid.getSelectionModel().getSelectedItem()!=null) {
			...
			try {
				...
				for(Employee dd: employeess) {
					dpnames.add(String.format("%s %s[%s](%d)", dd.getFirstName(),
							dd.getLastName(),dd.getEmail(),dd.getEmployeeId()));
				}
				//관리자 콤보에 관리자 아이디를 넣기
...
//			alert2.setContentText(new SQLException("Null Reference Exception").toString());
//			alert2.showAndWait();
			return;
		}
		...
		if(!empmanid.equals("")) {
			emp.setManagerId(Integer.parseInt(empmanid));
		}
