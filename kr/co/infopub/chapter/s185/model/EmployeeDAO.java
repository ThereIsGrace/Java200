...
//		" GROUP BY E.DEPARTMENT_ID                   ";
	...
	public List<DepCountDto> findAllDepCounts() throws SQLException{
		...
		//List<DepCountDto> empList = FXCollections.observableArrayList();
		try {
			conn = getConnection();
			psmt = conn.prepareStatement(SQL);
	...
	public List<EmployeeDto> findAllEmployees() throws SQLException{
		...
		try {
			...
			while(rs.next()) {
				...
				emp.setManager_id(rs.getInt("DEPARTMENT_ID"));
				empList.add(emp);
			}
	...
	public List<EmployeeDto> findTreeManagerInEmployee () throws SQLException{
		//inlive view - select from (select)
		String SQL = ""
			 + " SELECT                       "
			+ " EMPLOYEE_ID,                 "
			 + " MANAGER_ID,                   "
		...
		try {
			...
			log("5/6 findEmployeesByMangerId Success!!!");
		} catch(SQLException e) {
			log("findAllEmployeesByManagerId Success!!!");
			throw e;
		} finally {
			close(conn, psmt, rs);
		}
	...
	public List<EmployeeDto> findEmployeesByManagerId(String empid) throws SQLException{
		...
		String SQL = "SELECT "
				...
				+ " WHERE D.EMPLOYEE_ID = E.EMPLOYEE_ID AND E.MANAGER_ID=?               ";
		Connection conn = null;
		PreparedStatement psmt = null;
		ResultSet rs = null;
		List<EmployeeDto> empList = new ArrayList<>();
		try {
			...
			rs = psmt.executeQuery();
			log("4/6 findExmployeesByManagerId Success!!!");
			while(rs.next()) {
				EmployeeDto emp = new EmployeeDto();
				emp.setEmployee_id(rs.getInt("EMPLOYEE_ID"));
				emp.setFirst_name(rs.getString("FIRST_NAME"));
			...
			log("findEmployeesByManagerId Error!!!");
			throw e;
		} finally {
			close(conn, psmt, rs);
		}
