...
public class EmployeeSearchFxController {
	...
	private void edit2(boolean b) {
		...
		if(b) {
			...
			tfcommis.setStyle("-fx-background-color: #0000ff");
			tfsalary.setStyle("-fx-background-color: #0000ff");
			tfdepartid.setStyle("-fx-background-color: #0000ff");
			tfjobid.setStyle("-fx-background-color: #0000ff");
			tfmanagerid.setStyle("-fx-background-color: #0000ff");
			tfsearchmanager.setStyle("-fx-background-color: #0000ff");
		}else {
	...
	void initialize() {
		...
		empIdColumn3.setCellValueFactory(cellData -> 
			cellData.getValue().employeeIdProperty().asObject());
		empNameColumn3.setCellValueFactory(cellData -> 
			cellData.getValue().firstNameProperty());
		empLastNameColumn3.setCellValueFactory(cellData -> 
			cellData.getValue().lastNameProperty());
		empEmailColumn3.setCellValueFactory(cellData ->  
			cellData.getValue().emailProperty());
		...
		employeeTable.setOnMouseClicked(e -> {
			if(employeeTable.getSelectionModel().getSelectedItems()!=null) {
				edit(false);
				edit2(false);
				clear();
	...
	void actionDepartClicked(ActionEvent event) {
		if(cbdepartid.getSelectionModel().getSelectedItem()!=null) {
			...
			try {
				...
				List<EmployeeDto> empllist = employeeDAO.findEmployeesByDepartName(departid);
...
//	}
	...
	void actionAddEmployee(ActionEvent event) {
		...
		System.out.println(String.format("%s,%s,%s,%s,%s",empfn,empln,empmail,
				emphire,empphone));
		System.out.println(String.format("%s,%s,%s,%s,%s",emppct,empsal,
				depid(empdepid),empjobid,depid(empmanid)));
		...
		try {
			...
			if(empid > 0) {
				...
				System.out.println(" findEmployeeById 등록된 emp 찾기------------------"+empid);
				if(empDat!=null) {
					empData.add(empDat);
